...
int main(void) {
	...
	if (NULL != p_file1) {
		...
		if (file_size < ftell(p_file1)) {
			file_size = ftell(p_file1);//현재 위치로 파일의 크기를 구함
			winner = "fprintf.c";
	...
	if (NULL != p_file2) {
		...
		if (file_size < ftell(p_file2)) {
			file_size = ftell(p_file2);//현재 위치로 파일의 크기를 구함
			winner = "fscanf.c";
	...
	if (NULL != p_file3) {
		...
		if (file_size < ftell(p_file3)) {
			file_size = ftell(p_file3);//현재 위치로 파일의 크기를 구함
			winner = "fgets.c";
	...
	if (NULL != p_file4) {
		...
		if (file_size < ftell(p_file4)) {
			file_size = ftell(p_file4);//현재 위치로 파일의 크기를 구함
			winner = "fwrite.c";
	...
	if (NULL != p_file5) {
		...
		if (file_size < ftell(p_file5)) {
			file_size = ftell(p_file5);//현재 위치로 파일의 크기를 구함
			winner = "fread.c";
	...
	if (NULL != p_file6) {
		...
		if (file_size < ftell(p_file6)) {
			file_size = ftell(p_file6);//현재 위치로 파일의 크기를 구함
			winner = "fseek.c";
